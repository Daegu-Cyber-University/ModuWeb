...
if (window.WAT) {
	...
	class WAT {
		...
		_addEventListener(element, eventType, handlerKey, options = {}) {
			...
			if (scrollBlockingEvents.includes(eventType) && !options.hasOwnProperty('passive')) {
		...
		async performDiction(word) {
			...
			const timeout = this.getConfigValue('api.dictionary.timeout', 5000);
			const fullUrl = `${serverEndpoint}?word=${encodeURIComponent(cleanedWord)}`;
